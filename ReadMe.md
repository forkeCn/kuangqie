...
### 开始安装
##### 1，复制install.sh uninstall.sh到服务器，执行 chmod +x install.sh，执行./install.sh开始安装服务端，等待程序执行完，在浏览器输入(http://127.0.0.1:2333), 如看到以下登录页面，表示服务端已经成功安装，输入账号admin，密码admin，进入系统，卸载服务端执行./uninstall.sh
...
##### 2,如果是刚装过系统的ubuntu机器，默认是不支持ssh功能的，矿且系统需要矿机支持ssh，所以矿机需要自行安装sshd服务，我们这边也会提供一个脚本，可以让设备一键安装开启ssh服务，并设置允许root用户登录

##### 3，关注开发进度及发布时间请扫码进群
![image](https://forke.oss-cn-hangzhou.aliyuncs.com/101554350367_.pic_hd.jpg)
